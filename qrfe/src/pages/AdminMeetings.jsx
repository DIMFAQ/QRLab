...
export default function AdminMeetingsPage() {
  ...
  const extractErrorMessage = (err) => {
    try {
      ...
      if (err.response) {
        return `HTTP ${err.response.status} — ${err.response.data?.message || JSON.stringify(err.response.data)}`;
      }
      if (err.request) {
        return 'No response received from server (network/CORS/backend down).';
      }
  ...
  const fetchMeetings = async () => {
    setLoading(true);
    setError('');
    console.log('[AdminMeetings] fetchMeetings: start');
    try {
      const { data } = await api.get('/admin/meetings');
      console.log('[AdminMeetings] fetchMeetings: success', data);
      if (!Array.isArray(data)) {
        console.warn('[AdminMeetings] fetchMeetings: expected array but got', data);
      ...
      console.error('[AdminMeetings] fetchMeetings: failed', err);
      setError(msg);
      setMeetings([]);
      setActiveQr(null);
    } finally {
      setLoading(false);
      console.log('[AdminMeetings] fetchMeetings: end');
    }
  };

  const fetchActiveQr = async (meetingId) => {
    console.log('[AdminMeetings] fetchActiveQr', meetingId);
    try {
      const { data } = await api.get(`/admin/meetings/${meetingId}/active-qr`);
      console.log('[AdminMeetings] fetchActiveQr: success', data);
      setActiveQr(data);
    } catch (err) {
      console.warn('[AdminMeetings] fetchActiveQr: failed', err);
      setActiveQr(null);
    }
  };

  const handleMeetingCreated = (payload) => {
    console.log('[AdminMeetings] handleMeetingCreated', payload);
    fetchMeetings();
    setActiveQr(payload);
  };
  ...
  const handleCloseMeeting = async (meetingId) => {
    ...
    try {
      ...
      const msg = extractErrorMessage(err);
      console.error('[AdminMeetings] handleCloseMeeting failed', err);
      setError(msg);
    }
  };
  ...
  const handleShowRekap = async (meetingId) => {
    try {
      ...
      const msg = extractErrorMessage(err);
      console.error('[AdminMeetings] handleShowRekap failed', err);
  ...
  return (
    <div className="min-h-screen bg-blue-50 text-slate-900 pb-10">
      <div className="container mx-auto p-4">
        <AdminUserVerification />
      </div>

      <header className="sticky top-0 z-10 bg-blue-600 text-white px-5 py-3 shadow">
        <div className="max-w-3xl mx-auto flex justify-between items-center">
          <h1 className="text-lg font-semibold">Admin • QR Presensi</h1>
          <span className="text-xs bg-white/20 px-2 py-1 rounded-full">v1.0</span>
        </div>
      </header>

      <main className="max-w-3xl mx-auto p-4">
        <section className="rounded-2xl bg-white p-5 shadow ring-1 ring-slate-200 mb-6">
        ...
        {loading && (
          <div className="text-center text-sm text-slate-600 mb-4">Memuat data…</div>
        )}
